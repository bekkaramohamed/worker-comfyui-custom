...
CMD ["/bin/bash", "-c", "\
    ...
    echo '🌳 Full container tree (3 levels):' && tree -a -L 3 / || true; \
    ...
    /bin/bash /start.sh || (echo '❌ /start.sh failed with code $?'; python /handler.py; sleep 60) \
"]
